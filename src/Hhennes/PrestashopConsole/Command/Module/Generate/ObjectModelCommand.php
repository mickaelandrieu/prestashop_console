...
class ObjectModelCommand extends Command
{
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        //Table name
        $tableQuestion = new Question('<question>Table name :</question>');
        $tableQuestion->setValidator(function ($answer) {
            if (!Validate::isTableOrIdentifier($answer)) {
                throw new \RuntimeException('The table name is not valid');
            }
            return $answer;
        });
        $tableName = $helper->ask($input, $output, $tableQuestion);
        $primaryQuestion = new Question('<question>Primary key :</question>');
        $primaryQuestion->setValidator(function ($answer) {
            if (!Validate::isTableOrIdentifier($answer)) {
                throw new \RuntimeException('The identifier is not valid');
            }
            return $answer;
        });
        $primary = $helper->ask($input, $output, $primaryQuestion);
        ...
        do {
            ...
            //Field Name
            $fieldNameQuestion = new Question('<question>Field name :</question>');
            $primaryQuestion->setValidator(function ($answer) {
                if (!Validate::isTableOrIdentifier($answer)) {
                    throw new \RuntimeException('The identifier is not valid');
                }
            ...
            $name = $helper->ask($input, $output, $fieldNameQuestion);
            ...
            $newField = $helper->ask($input, $output,
                new ConfirmationQuestion('<question>Add another field ?</question>', false, '/^(y|j)/i')
            );
        ...
        //@Todo Generate sql request in install file
        //Ask for create a new field
        $sql = $helper->ask($input, $output,
            new ConfirmationQuestion('<question>Generate sql ?</question>', false, '/^(y|j)/i')
        );
        if ($sql) {
            $this->_generateSql($params);
        }

        //@Todo : Generate admin controller for model
    }
